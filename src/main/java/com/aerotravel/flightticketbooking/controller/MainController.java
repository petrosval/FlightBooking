...
public class MainController {
    ...
    public String registerUser(@Valid @ModelAttribute("user") User user, BindingResult bindingResult, Model model) {
        ...
        roles.add(role);
        User user1 = new  User(user.getFirstname(),user.getLastname(),user.getUsername(),passwordEncoder.encode(user.getPassword()),user.getCountry(),user.getCity(),user.getAddress(),user.getEmail(), roles);
        //Role role = new Role();
       // role.setId();
       // role.setName();
    ...
    private void sendmail(Passenger emailPassenger , Flight emailFlight) throws AddressException, MessagingException, IOException {
        ...
        Session session = Session.getInstance(props, new javax.mail.Authenticator() {
            protected PasswordAuthentication getPasswordAuthentication() {
                return new PasswordAuthentication("aeroreservasions@gmail.com", "valacheas21");
            }
        });
        Message msg = new MimeMessage(session);
        msg.setFrom(new InternetAddress("aeroreservasions@gmail.com", false));

        msg.setRecipients(Message.RecipientType.TO, InternetAddress.parse(emailPassenger.getEmail()));
        msg.setSubject("Flight Booking from Aero ");
        msg.setContent("Flight Booking from Aero", "text/html");
        msg.setSentDate(new Date());

        MimeBodyPart messageBodyPart = new MimeBodyPart();
        messageBodyPart.setContent("Thank you " + emailPassenger.getFirstName() + " " + emailPassenger.getLastName()+ " for Booking a Flight with ID: "+ emailFlight.getFlightId() +" and Flight_Number: "+emailFlight.getFlightNumber(), "text/html");
